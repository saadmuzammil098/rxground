...
| 7 (bonus) | [`task-7/`](./task-7) | Query service: a thin FastAPI wrapper (`POST /check_interaction`) around task-3's citation-enforced `answer_question`, unmodified, so [CareThread](../carethread) (the roadmap's capstone) can make a genuine live HTTP call into RxGround for drug-interaction checks instead of a static lookup table. Verified live: a warfarin/aspirin query correctly refuses (neither drug's exact pairing is indexed), a Warfarin Sodium/ZOCOR query returns a real grounded answer citing warfarin's actual label chunk |
